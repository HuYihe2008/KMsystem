...
def init_db(update=False):
    ...
    # 支付渠道
    db.session.add(Payment('支付宝当面付', '支付宝', "{'APPID':'xxxx','alipay_public_key':'xxx','app_private_key':'xxx'}", 'alipay.com 官方接口0.38~0.6%', False))
    ...
    db.session.add(Config(
        'web_footer', '<a style="color: #fafafa;" href="https://github.com/HuYihe2008/stcxzx202001">鲁ICP备1101XXXX号-10</a>', '可填写备案信息', True))
    ...
    db.session.add(Config('theme', 'list', '主题', True))
    db.session.add(Config(
        'Jamyidofaka', '#', 'Github项目地址，用于手动检测新版', True))
    db.session.add(Config('Jamyidofaka_v', '1.0', 'Github项目地址，用于手动检测新版', True))
